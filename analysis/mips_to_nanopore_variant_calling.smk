...
def get_samples_for_targeted_calling_from_bam_coverage():
    coverage_file = os.path.join(config['output_dir'], 'samples_for_targeted_calling', 'bam_with_coverage.txt')
    sample_names = []
    with open(coverage_file, 'r') as f:
        for line in f:
            # extract the basename of the file (eg. RW13013-105105-ArtR22RW-1.sorted.bam)
            bam_basename = os.path.basename(line.strip())
            # assuming the sample name is before ".sorted.bam", split and take the first element
            sample_name = bam_basename.split('.sorted.bam')[0]
...
rule run_clair3_targets:
	...
	"""
	input:
		samples_to_run=config["output_dir"]+'/mapping/bam_files/{samples_for_targeted_calling}.sorted.bam',
		REF = config['genome_directory']+'/genome.fa', # TODO:make demo resources with Pf3D7 genome for built in tutorial demo data
		sif_path = config["clair3_sif_path"] # TODO: this can be handled by the singularity container eventually so users dont install clair3 themselves.
	output:
		vcf=config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.vcf.gz',
		gvcf=config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.gvcf.gz'
	params:
		INPUT_DIR = config["output_dir"]+'/mapping/bam_files',
		OUTPUT_DIR = config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets',
		REF_DIR = config['genome_directory'],
		RESOURCE_DIR = config["project_resources"],
		THREADS = 4,
		MODEL_NAME = 'r941_prom_sup_g5014',
	...
	shell:
		'''
		echo 'Processing samples_for_targeted_calling: {wildcards.samples_for_targeted_calling}'
		...
		singularity exec \
			...
			--include_all_ctgs \
			--no_phasing_for_fa \
			--sample_name={wildcards.samples_for_targeted_calling} \
			--gvcf \
			--include_all_ctgs \
			--threads=1 \
			--snp_min_af=0.0 \
...
rule bcftools_index_gvcf_targets:
	input:
		original_gvcfs_to_index_targets=config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.gvcf.gz'
	output:
		index_file_gvcf_targets=config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.gvcf.gz.csi'
	shell:
		'''
...
rule bcftools_index_vcf_targets:
	input:
		original_vcfs_to_index_targets=config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.vcf.gz'
	output:
		index_file_vcf_targets=config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.vcf.gz.csi'
	shell:
		'''
...
rule prepare_gvcf_lists:
	input: 
		#finished_index_file_gvcf_targets=config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.gvcf.gz.csi',
		finished_index_file_gvcf_targets=expand(config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.gvcf.gz.csi',samples_for_targeted_calling=get_samples_for_targeted_calling_from_bam_coverage()),
	...
	output:
		raw_list = temp(config["output_dir"] + '/gvcf_paths_raw.txt'),
		targets_list = temp(config["output_dir"] + '/gvcf_paths_targets.txt')
	params:
...
rule prepare_vcf_lists:
	input: 
		finished_index_file_vcf_targets=expand(config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.vcf.gz.csi',samples_for_targeted_calling=get_samples_for_targeted_calling_from_bam_coverage()),
	...
	output:
		raw_list = temp(config["output_dir"] + '/vcf_paths_raw.txt'),
		targets_list = temp(config["output_dir"] + '/vcf_paths_targets.txt')
	params:
...
rule bcftools_merge_targets_gvcf:
	input:
		original_gvcfs_targets=expand(config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.gvcf.gz.csi',samples_for_targeted_calling=get_samples_for_targeted_calling_from_bam_coverage()),
...
rule bcftools_merge_targets_vcf:
	input:
		original_vcfs=expand(config["output_dir"]+'/variant_calling/{samples_for_targeted_calling}_targets'+'/merge_output.vcf.gz.csi',samples_for_targeted_calling=get_samples_for_targeted_calling_from_bam_coverage()),
